...
function removeLink(element) {
	...
	if (eventInputID.length){
		$.ajax({
			type: 'GET',  
			url: 'https://k12start.today:'+port+'/DeleteEventQuickLink/',
		...
		$.ajax({
			type: 'GET',  
			url: 'https://k12start.today:'+port+'/DeleteProjectLink/',
			contentType: "application/json; charset=utf-8",
		...
		$.ajax({
			type: 'GET',  
			url: 'https://k12start.today:'+port+'/deleteLink/',
			contentType: "application/json; charset=utf-8",
	...
	$('#link-submit-btn').click(function(event) {
		...
		if (linkList.length < 16) {
			if ($.inArray(httpURL, linkList) == -1) {
				$.ajax({
					type: 'GET',  
					url: 'https://k12start.today:'+port+'/addLink/',
					contentType: "application/json; charset=utf-8",
					data: {userLink:httpURL, userLinkDesc:userlinkdesc},
					success: function(data) {
	...
	$('#link-event-submit-btn').click(function(event) {
		...
		if (linkList.length < 16) {
			if ($.inArray(httpURL, linkList) == -1) {
				$.ajax({
					type: 'GET',
					url: 'https://k12start.today:'+port+'/AddEventQuickLink/',
					contentType: "application/json; charset=utf-8",
	...
	$('#link-project-submit-btn').click(function(event) {
		...
		if (linkList.length < 16) {
			if ($.inArray(httpURL, linkList) == -1) {
				$.ajax({
					type: 'GET',
					url: 'https://k12start.today:'+port+'/AddProjectLink/',
					contentType: "application/json; charset=utf-8",
